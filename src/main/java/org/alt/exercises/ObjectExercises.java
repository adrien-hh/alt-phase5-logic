...
public class ObjectExercises {
  ...
  public static <V, R> Map<String, Object> transformValues(
      Object obj, Class<V> type, Function<V, R> transformer) {
    Map<String, Object> result = new LinkedHashMap<>();
  ...
  public static <V> Map<String, Object> filterObject(
      Object obj, Class<V> type, Predicate<V> predicate) {
    Map<String, Object> result = new LinkedHashMap<>();
    for (Map.Entry<String, Object> entry : getValues(obj).entrySet()) {
      if (type.isInstance(entry.getValue())) {
  ...
  public static Map<String, Object> flatToNested(Map<String, Object> flatMap) {
    ...
    for (Map.Entry<String, Object> entry : flatMap.entrySet()) {
      ...
      for (int i = 0; i < keys.length - 1; i++) {
        current =
            (Map<String, Object>) current.computeIfAbsent(keys[i], k -> new LinkedHashMap<>());
      }
  ...
  public static Map<String, Map<String, Double>> getObjectStats(Object obj) {
    ...
    double median =
        (n % 2 == 0) ? (values.get(n / 2 - 1) + values.get(n / 2)) / 2.0 : values.get(n / 2);
    double variance =
        values.stream().mapToDouble(v -> Math.pow(v - average, 2)).average().orElse(0);
  ...
  public static void run() {
    ...
    System.out.println(
        "5. flatToNested:\n\t"
            + flatToNested(
                Map.of("app.name", "MyApp", "app.version", "1.0.0", "database.host", "localhost")));
